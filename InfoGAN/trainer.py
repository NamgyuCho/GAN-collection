# Code from https://github.com/pianomania/infoGAN-pytorch/blob/master/trainer.py
import torch
...
class Trainer:
    ...
    def train(self):
        ...
        dataset = dset.MNIST('./dataset', transform=transforms.ToTensor(), downlaod=True)
        ...
        fix_noise = torch.Tensor(100, 62).uniform(-1, 1)
        ...
        for epoch in range(100):
            for num_iters, batch_data in enumerate(dataloader, 0):
                ...
                dis_c.data.resize(bs, 10)
                con_c.data.resize(bs, 2)
                noise.data.resize(bs, 62)
                ...
                if num_iters % 100 == 0:
                    ...
                    z = torch.cat([noise, dis_c, con_c], 1).view(-1, 74, 1, 1)
                    x_save = self.G(z)
                    save_image(x_save.data, './tmp/c1_epoch{0}.png'.format(num_iters), nrow=10)

                    con_c.data.copy_(torch.from_numpy(c2))
                    z = torch.cat([noise, dis_c, con_c], 1).view(-1, 74, 1, 1)
                    x_save = self.G(z)
                    save_image(x_save.data, './tmp/c2_epoch{0}.png'.format(num_iters), nrow=10)
